...
- **P2 reconectado — reconciliación surplus ↔ Δ patrimonio**: `components/SurplusReconciliation.tsx`, renderizado en `/analisis` (no como banner diario en `/inicio`, que se mantiene desactivado por intrusivo). Para un mes ya CERRADO con sobrante positivo, compara `surplus` (sueldo M-1 − gasto de M) contra el Δ `net_clp` del mismo mes (usando los snapshots historizados por el fix de arriba) y muestra la diferencia sin explicar: "sobraron $X pero tu patrimonio subió $Y — ¿dónde están los otros $Z?". Reutiliza `recordMonthSweep`/`month_sweeps` que ya estaban montados y sin uso. Si el patrimonio subió tanto o más que el sobrante, o si faltan snapshots para comparar (meses previos al fix), no se muestra — no castiga por falta de dato.
